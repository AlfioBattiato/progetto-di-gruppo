  ...
  (async () => {
    ...
    function sbloccaResultPage() {
      ...
      pCorrect.textContent =
        "Risposte corrette : " + punteggioCorrette + " / " + questions.length;
      pIncorrect.textContent =
        "Risposte sbagliate : " + punteggioSbagliate + " / " + questions.length;

      punteggioSbagliate = Math.floor(
        (punteggioSbagliate / questions.length) * 100
      );
      punteggioCorrette = Math.floor(
        (punteggioCorrette / questions.length) * 100
      );
...
const fetchQuestions = async (n, d) => {
  try {
    const response = await fetch(
      `https://opentdb.com/api.php?amount=${n}&category=18&difficulty=${d}`
    );
    if (response.ok) {
